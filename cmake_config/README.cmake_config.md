...
```bash
\curl -L 'https://github.com/mooreryan/sample_seqs/archive/v@SMPL_VERSION_STRING@.tar.gz' > v@SMPL_VERSION_STRING@.tar.gz
```

If you have `wget`, run

```bash
wget 'https://github.com/mooreryan/sample_seqs/archive/v@SMPL_VERSION_STRING@.tar.gz'
```

Unzip the tar file and enter the resulting directory.

```bash
tar xzf v@SMPL_VERSION_STRING@.tar.gz
cd sample_seqs-@SMPL_VERSION_STRING@
```
